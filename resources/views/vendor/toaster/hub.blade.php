<div role="status" id="toaster" x-data="toasterHub(@js($toasts), @js($config))" @class([
    'fixed z-[9999] p-4 w-full flex flex-col pointer-events-none sm:p-6',
    ...
    <template x-for="toast in toasts" :key="toast.id">
        <div x-show="toast.isVisible"
             ...
             @class(['relative duration-300 transform transition ease-in-out max-w-sm w-full pointer-events-auto', 'text-center' => $position->is('center')])
             :class="toast.select({ error: 'text-white', info: 'text-black', success: 'text-white', warning: 'text-white' })"
        >
            <i x-text="toast.message"
               class="inline-block select-none not-italic px-6 py-3 rounded shadow-lg text-md w-full {{ $alignment->is('bottom') ? 'mt-3' : 'mb-3' }}"
